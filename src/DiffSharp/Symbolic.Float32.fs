...
module ExprOps =
    ...
    let rec diffExpr (v:Var) expr =
        match expr with
        | Double(_) -> <@@ 0. @@>
        | SpecificCall <@ (+) @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ (%%at:number) + %%bt @@>
        | SpecificCall <@ (-) @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ (%%at:number) - %%bt @@>
        | SpecificCall <@ (*) @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ (%%at:number) * %%b + %%a * %%bt @@>
        | SpecificCall <@ (/) @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ ((%%at:number) * %%b - %%a * %%bt) / (%%b * %%b)@@>
        | SpecificCall <@ op_Exponentiation @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ ((%%a:number) ** %%b) * ((%%b * %%at / %%a) + ((log %%a) * %%bt)) @@>
        | SpecificCall <@ atan2 @> (_, _, [a; b]) -> let at, bt = diffExpr v a, diffExpr v b in <@@ ((%%at:number) * %%b - %%a * %%bt) / (%%a * %%a + %%b * %%b) @@>
        | SpecificCall <@ (~-) @> (_, _, [a]) -> let at = diffExpr v a in <@@ -(%%at:number) @@>
        | SpecificCall <@ log @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / %%a @@>
        | SpecificCall <@ log10 @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (%%a * log10ValFloat32) @@>
        | SpecificCall <@ exp @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * %%expr @@>
        | SpecificCall <@ sin @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * cos %%a @@>
        | SpecificCall <@ cos @> (_, _, [a]) -> let at = diffExpr v a in <@@ -(%%at:number) * sin %%a @@>
        | SpecificCall <@ tan @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / ((cos %%a) ** 2.f) @@>
        | SpecificCall <@ sqrt @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (2.f * %%expr) @@>
        | SpecificCall <@ sinh @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * cosh %%a @@>
        | SpecificCall <@ cosh @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * sinh %%a @@>
        | SpecificCall <@ tanh @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / ((cosh %%a) ** 2.f) @@>
        | SpecificCall <@ asin @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / sqrt (1.f - %%a * %%a) @@>
        | SpecificCall <@ acos @> (_, _, [a]) -> let at = diffExpr v a in <@@ -(%%at:number) / sqrt (1.f - %%a * %%a) @@>
        | SpecificCall <@ atan @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (1.f + %%a * %%a) @@>
        | SpecificCall <@ abs @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * (float32 (sign %%a)) @@> //The derivative of abs is not defined at 0.
    ...
    let simplify expr =
        let rec simplifyExpr expr =
            ...
            | SpecificCall <@ (-) @> (_, _, [a; Double(0.)]) -> a
            | SpecificCall <@ (-) @> (_, _, [Double(0.); b]) -> <@@ -(%%b:number) @@>
            | SpecificCall <@ (/) @> (_, _, [a; Double(1.)]) -> a
...
module DiffOps =
    ...
    let diff' f =
        let fe = (QuotationEvaluator.CompileUntyped f) :?> (number->number)
        let fd = diff f
        fun x -> (fe x, fd x)

    /// `n`-th derivative of a scalar-to-scalar function `f`
    let diffn n (f:Expr<number->number>) =
        let fe = expand f
        let args = getExprArgs fe
    ...
    let jacobianT (f:Expr) =
        ...
        fun x -> ShapedDataBufferView<number>(Array.map (evalVV x) fj |> Seq.concat |> Seq.toArray |> NativeDataBuffer<number>)
    ...
    let hessian (f:Expr) =
        ...
        fun (x:number[]) ->
            let ret:number[,] = Array2D.create x.Length x.Length 0.f
...
// might or might not get back to this later when I need it
