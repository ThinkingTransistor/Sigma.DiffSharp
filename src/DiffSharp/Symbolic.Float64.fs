...
module ExprOps =
    ...
    let rec diffExpr (v:Var) expr =
        ...
        | SpecificCall <@ log10 @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (%%a * log10ValFloat64) @@>
        ...
        | SpecificCall <@ tan @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / ((cos %%a) ** 2.) @@>
        | SpecificCall <@ sqrt @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (2. * %%expr) @@>
        ...
        | SpecificCall <@ tanh @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / ((cosh %%a) ** 2.) @@>
        | SpecificCall <@ asin @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / sqrt (1. - %%a * %%a) @@>
        | SpecificCall <@ acos @> (_, _, [a]) -> let at = diffExpr v a in <@@ -(%%at:number) / sqrt (1. - %%a * %%a) @@>
        | SpecificCall <@ atan @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) / (1. + %%a * %%a) @@>
        | SpecificCall <@ abs @> (_, _, [a]) -> let at = diffExpr v a in <@@ (%%at:number) * float (sign %%a) @@> //The derivative of abs is not defined at 0.
...
module DiffOps =
    ...
    let hessian (f:Expr) =
        ...
        fun (x:number[]) ->
            let ret:number[,] = Array2D.create x.Length x.Length 0.
