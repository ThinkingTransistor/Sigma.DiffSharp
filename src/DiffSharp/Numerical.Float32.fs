...
let inline FixedPointEpsilonRec2() = global.DiffSharp.Config.GlobalConfig.Float32EpsilonRec2
...
module DiffOps = 
    ...
    let inline diff' f x = (f x, diff f x)
    ...
    let inline grad' (f : IDataBuffer -> number) x (backend : Backend<number>) = 
        ...
        let gg = 
            NativeDataBuffer<number>
                (Array.init x.Length 
                     (fun i -> 
                     f (backend.Add_V_V(x, NativeDataBuffer<number>(standardBasisVal x.Length i (FixedPointEpsilon()))))))
        (fx, backend.Mul_S_V(FixedPointEpsilonRec(), backend.Sub_V_V(gg, g)))
    
    /// Gradient of a vector-to-scalar function `f`, at point `x`
    let grad f x (backend : Backend<number>) = grad' f x backend |> snd
    ...
    let inline gradhessian' f x (backend : Backend<number>) = 
        ...
        let h = 
            ShapedDataBufferView(NativeDataBuffer<number>((Array.create x.Length (Array.toSeq g.SubData))
                                                          |> Seq.concat
                                                          |> Seq.toArray), int64 x.Length, int64 x.Length)
        
        let hh = 
            ShapedDataBufferView((Array.init x.Length 
                                      (fun i -> 
                                      Array.toSeq 
                                          ((grad f 
                                                (backend.Add_V_V
                                                     (x, 
                                                      NativeDataBuffer<number>
                                                          (standardBasisVal x.Length i (FixedPointEpsilon())))) backend).SubData)))
                                 |> Seq.concat
                                 |> Seq.toArray
                                 |> NativeDataBuffer<number>, int64 x.Length, int64 x.Length)
    ...
    let inline gradhessian f x (backend : Backend<number>) = gradhessian' f x backend |> sndtrd
    
    /// Original value and Hessian of a vector-to-scalar function `f`, at point `x`
    let inline hessian' f x (backend : Backend<number>) = gradhessian' f x backend |> fsttrd
    
    /// Hessian of a vector-to-scalar function `f`, at point `x`
    let inline hessian f x (backend : Backend<number>) = gradhessian' f x backend |> trd
    ...
    let inline hessianv (f : IDataBuffer -> number) x v (backend : Backend<number>) = hessianv' f x v backend |> snd
    ...
    let inline laplacian' f x (backend : Backend<number>) = 
        let (v, h) = hessian' f x backend
        (v, backend.Sum_V(backend.Diagonal_M(h)))
    
    /// Laplacian of a vector-to-scalar function `f`, at point `x`
    let inline laplacian f x (backend : Backend<number>) = laplacian' f x backend |> snd
    ...
    let inline jacobianT' (f : IDataBuffer -> IDataBuffer) x (backend : Backend<number>) = 
        ...
        let j = 
            ShapedDataBufferView<number>(NativeDataBuffer<number>((Array.create x.Length (Array.toSeq fx.SubData))
                                                                  |> Seq.concat
                                                                  |> Seq.toArray), int64 x.Length, int64 x.Length)
        
        let jj = 
            ShapedDataBufferView<number>(Array.init x.Length 
                                             (fun i -> 
                                             Array.toSeq 
                                                 ((f 
                                                       (backend.Add_V_V
                                                            (x, 
                                                             NativeDataBuffer<number>
                                                                 (standardBasisVal x.Length i (FixedPointEpsilon()))))).SubData))
                                         |> Seq.concat
                                         |> Seq.toArray
                                         |> NativeDataBuffer<number>, int64 x.Length, int64 x.Length)
    ...
    let inline jacobianT f x (backend : Backend<number>) = jacobianT' f x backend |> snd
    ...
    let inline jacobian f x (backend : Backend<number>) = jacobian' f x backend |> snd
    
    /// Jacobian-vector product of a vector-to-vector function `f`, at point `x`, along vector `v`
    let inline jacobianv (f : IDataBuffer -> IDataBuffer) x v (backend : Backend<number>) = 
        let veps = backend.Mul_S_V(FixedPointEpsilon(), v)
        backend.Mul_S_V
            (FixedPointEpsilonRec2(), backend.Sub_V_V(f (backend.Add_V_V(x, veps)), f (backend.Sub_V_V(x, veps))))
    
    /// Original value and Jacobian-vector product of a vector-to-vector function `f`, at point `x`, along vector `v`
    let inline jacobianv' f x v (backend : Backend<number>) = (f x, jacobianv f x v)
    ...
    let inline curl f x (backend : Backend<number>) = curl' f x backend |> snd
    ...
    let inline div f x (backend : Backend<number>) = div' f x backend |> snd
    ...
    let inline curldiv f x (backend : Backend<number>) = curldiv' f x backend |> sndtrd
